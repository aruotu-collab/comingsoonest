...
const outPath = path.join(root, "src", "data", "catalogue.generated.json");
...
const payload = { brands, launches };
fs.writeFileSync(outPath, JSON.stringify(payload), "utf8");
console.log(
  `Wrote ${brands.length} brands and ${launches.length} launches → ${path.relative(root, outPath)} (${(Buffer.byteLength(JSON.stringify(payload)) / 1024 / 1024).toFixed(2)} MB)`
);
